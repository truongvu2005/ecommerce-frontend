...
export default function Header() {
  ...
  // Hàm xử lý khi bấm Enter hoặc nút Tìm kiếm
        ...
        <div className="hidden md:flex flex-1 max-w-md mx-8">
          <form onSubmit={handleSearch} className="relative w-full">
            <input 
              ...
              className="w-full bg-slate-100 border-none rounded-full py-2.5 pl-6 pr-12 outline-none focus:ring-2 focus:ring-blue-500 transition font-medium text-slate-700" 
            />
        ...
        <div className="flex items-center gap-4">
          {currentUser ? (
            ...
            <div className="flex gap-3">
              <Link href="/register" className="hidden sm:block px-5 py-2 rounded-full font-bold text-blue-600 hover:bg-blue-50 transition">
                Đăng ký
              </Link>
              <Link href="/login" className="bg-blue-600 text-white px-5 py-2 rounded-full font-bold hover:bg-blue-700 shadow-md transition transform hover:-translate-y-0.5">
                Đăng Nhập
              </Link>
            </div>
          )}
